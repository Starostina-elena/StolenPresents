...
# функция проверки поля на наличие победителя
def check_field(field):
    global status_win

    n = len(field)
    is_winner = False
    # проверка всех ситуаций
    for i in range(n):
        if not is_winner:
...
class Board:
    ...
    def bot_turn(self):
        n = len(self.board)
        is_turn = False
        # ниже идут всевозможные случаи хода
        if not (is_turn):
            for i in range(n):
...
def show_rules(message=None):
    ...
    if message:
        ...
        rules = pygame_gui.windows.UIMessageWindow(
            ...
            html_message='<font color="black">Играйте с компьютером: соберите '
                         'ряд из трех крестиков, чтобы победить!'
        )
    # настройка внешнего вида
    rules.dismiss_button.text = 'Закрыть'
    rules.dismiss_button.colours['normal_bg'] = pygame.Color(
        (240, 240, 240, 255))
    rules.dismiss_button.colours['hovered_bg'] = pygame.Color(
        (255, 255, 255, 255))
    rules.dismiss_button.colours['active_bg'] = pygame.Color(
        (255, 255, 255, 255))
    rules.dismiss_button.colours['normal_border'] = pygame.Color(
        (255, 255, 255, 0))
    rules.dismiss_button.colours['hovered_border'] = pygame.Color(
        (0, 255, 0, 255))
    rules.dismiss_button.colours['normal_text'] = pygame.Color(
        (255, 40, 40, 255))
    rules.dismiss_button.colours['hovered_text'] = pygame.Color(
        (255, 40, 40, 255))
